...
const Login = () => {
  ...
  const handleSubmit=async(e)=>{
    e.preventDefault();
    try {
       const response=await axios.post('https://ems-backend-dun.vercel.app/api/auth/login',{email,password});
      ...
      <div className='border shadow p-6 w-80 bg-white-800'>
        ...
        <form onSubmit={handleSubmit}>
          <div className='mb-4'>
            ...
            <input required className='w-full px-3 py-2 border' type='email' placeholder='Enter Email' 
               onChange={(e)=>setEmail(e.target.value)
            }  />
          </div>
          <div>
            <label htmlFor='password'></label>
            <input required className='w-full px-3 py-2 border' type='password' placeholder='*****' 
